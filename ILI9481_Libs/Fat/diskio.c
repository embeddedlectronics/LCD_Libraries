...
/*-----------------------------------------------------------------------*/
/* Only rcvr_spi(), xmit_spi() and some macros         */
/* are platform dependent.                                               */
/*-----------------------------------------------------------------------*/

// heavily modified by Frank Zhao
...
#define CS_LOW()	PORTB &= ~_BV(0)			/* MMC CS = L */
#define	CS_HIGH()	PORTB |= _BV(0)			/* MMC CS = H */
...
)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			for (int i = 0; i < 1000; i++)
            {
                if (send_cmd(cmd, 0) != 0)			/* Wait for leaving idle state */
                {
                    break;
                }
            }
